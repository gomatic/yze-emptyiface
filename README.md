...
A [`yze`](https://github.com/gomatic/yze) analyzer (group `go`, category `modern-go`) enforcing the gomatic Go standard that the empty interface is written as `any`, not `interface{}`. It offers a **mechanical fix** (`yze --fix` rewrites `interface{}` → `any`; `gopls` surfaces it as a quick-fix).

- **Rule:** `yze/go/emptyiface`
